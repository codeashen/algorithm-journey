...
- l：进行 partition 操作最左边元素的下标，对应元素 v 即为基点
- i：被遍历访问的元素 e 的下标，从 l+1 开始
- j：小于 v 和大于 v 区的分界点，指向小区间的最右边元素下标

![image-20210813195326592](https://z3.ax1x.com/2021/08/13/frOXTO.png)

1. 通常选择数组的第一个索引位 l 处的元素 v 作为标的
2. 指定 j=l，i=l+1，开始从左往右遍历元素 e
3. 如果 e > v，则直接将元素放到大区间尾部，继续遍历下一个元素（对应代码里啥都不用做）
4. 如果 e < v，则将元素 e 和 j+1 指向的元素交换，然后 j++，继续遍历下一元素（对用代码里，swap(arr, ++p, i)）

![image-20210813201737320](https://z3.ax1x.com/2021/08/13/frzNa6.png)

5. 遍历完成后数组被分成三部分，首部基点元素 v，小区间和大区间

![image-20210813201556298](https://z3.ax1x.com/2021/08/13/frzVrn.png)

6. 最后只需要将 l 和 j 下标的元素交换，此时 partition 操作完成，基点 v 分隔了小区间和大区间
